...
public class AuthService {
    ...
    public void authenticateUser(AuthRequest authRequest){
        User user = userRepo.findByEmail(authRequest.email())
                .orElseThrow(()-> new AuthenticationException("Неправильный пароль или почта"));
        if (!passwordEncoder.matches(authRequest.password(), user.getPassword()))
            throw new AuthenticationException("Неправильный пароль или почта");
    }

    public JwtResponse login(@NonNull AuthRequest authRequest) {
        try {
            authenticateUser(authRequest);
            User user = userRepo.findByEmail(authRequest.email()).orElseThrow();
